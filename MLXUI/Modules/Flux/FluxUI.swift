...
/// `ModelUI` for FLUX.1-family text → image. Presents `FluxRunView` (prompt + seed + size +
/// Run → image preview + Save PNG). AM5.
struct FluxUI: ModelUI {
    ...
    @MainActor func makeRunView(for model: ModelEntry, stage: any PipelineStage) -> AnyView {
        AnyView(FluxRunView(modelDisplayName: model.displayName,
                            license: model.license,
                            modelID: model.id))
    }
}

/// The run surface for FLUX text → image: a prompt, a seed (for reproducible output), an
/// image-size picker, Run → live progress → image preview → Save PNG.
struct FluxRunView: View {
    ...
    @State private var model = FluxRunModel()
    @State private var prompt = "An astronaut riding a horse on Mars."
    @State private var seedText = "42"
    @State private var sizeIndex = 1

    private let sizes: [(Int, Int, String)] = [
        (512, 512, "512×512"),
        (768, 768, "768×768"),
        (1024, 1024, "1024×1024"),
    ]
    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text("Text → image").font(.caption).foregroundStyle(.secondary)
            }
            Spacer()
        }
    }
    ...
    private var progressRow: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(model.progress < 0.5 ? "Loading weights…" : "Generating (50-step denoise)…")
                .font(.caption).foregroundStyle(.secondary)
        }
        .padding(10)
        .background(.quaternary.opacity(0.4), in: RoundedRectangle(cornerRadius: 8))
    ...
    private func run() {
        ...
        let stage = FluxStage(modelID: modelID, seed: seed)
    ...
    private func save(_ image: CGImage) {
        ...
        panel.nameFieldStringValue = "flux.png"
...
/// View-model driving one FLUX run: runs the stage off-actor and folds progress + the image
/// back onto the main actor. Mirrors `TTSRunModel`/`OCRRunModel`.
@MainActor
@Observable
final class FluxRunModel {
    var isRunning = false
    var progress = 0.0
    var image: CGImage?
